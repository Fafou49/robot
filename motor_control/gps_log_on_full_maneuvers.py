...
continuously for as long as EITHER maneuver was being logged
(link.gamepad_handler.GamepadReader.start_rumble(), wired via
MultiConditionGPSLogger's on_transition/on_gps_quality callbacks below,
strong or weak depending on live GPS fix quality, with an
active-condition counter so back-to-back maneuvers didn't cause a
spurious buzz/pause/buzz) -- that vibration wiring has been removed (this
field-test tool no longer touches the gamepad's rumble motor at all). See
link/gamepad_handler.py's module docstring and link/gps_reader.py for the
vibration feature that replaced it: a short, one-shot pulse on
link/server.py's REAL control link the instant the live GPS fix's
quality actually changes during real operation, not a continuous buzz
tied to a field-test maneuver like this one. gps_condition_logger.py's
on_transition/on_gps_quality hooks themselves still exist (unused by
this script now) in case a future feature wants them again.
