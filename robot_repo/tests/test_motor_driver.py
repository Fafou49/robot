...
# gpiod (and gpiod.line) is stubbed in tests/conftest.py -- loaded by
# pytest before any test module in this directory is collected, so this
# file just relies on sys.modules already having it by the time this
# import runs. See conftest.py's own docstring for why it moved there
# (2026-09-11): a stub set up here, inline, only protects test files
# collected AFTER this one -- tests/test_link_server.py, sorting earlier,
# imports motor_control.motor_driver for real via link.server first,
# permanently caching motor_driver._GPIOD_AVAILABLE=False before this
# file's stub (previously set up right here) ever ran.
